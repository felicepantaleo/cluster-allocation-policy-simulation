...
4. Every allocation beyond the first GPU declares a maximum duration, at most 7 days, defaulting to 8 hours if none is given. It can be renewed without limit; each renewal competes again at the package's current priority. It may span several GPUs or several servers, for example a job that runs across many GPUs at once. A member can also request that its servers sit on one high-performance network switch and add a second server, with or without a GPU, on the same switch, so the two exchange data at full bandwidth.
